...
import { getCampaignHandler, generateStrategyHandler, generateModuleHandler, approveQcHandler, rejectQcHandler, getSourcesHandler, getQcReviewsHandler, getMessagesHandler, chatHandler, getExportEventsHandler, recordExportHandler, } from "./campaign-orchestrator";
const testBrief = {
    startupName: "TestCo",
...
describe("campaign-orchestrator handlers", () => {
    ...
    it("generateModuleHandler transitions to review_pending when QC finds issues", async () => {
        ...
        llmModule.completeStructuredPromptWithRecovery.mockResolvedValue({
    ...
    it("generateModuleHandler transitions to review_pending when a QC reviewer rejects", async () => {
        ...
        llmModule.completeStructuredPromptWithRecovery
    ...
    it("export ownership: user B cannot record export for user A campaign", async () => {
        const campaign = await createCampaign(db, { brief: testBrief, userId: "user-a" });
        await expect(recordExportHandler(campaign.id, "pdf", "user-b")).rejects.toThrow(`Campaign not found: ${campaign.id}`);
