...
import type { CampaignBrief, CampaignId, CampaignModuleId, CampaignModuleOutput, CampaignWorkflowState, McpSource, QcReviewResult, RunId } from "../../campaign/types";
...
export async function executeQcReviewNode(
  db: Db,
  input: {
  ...
  for (const [index, result] of reviewResults.entries()) {
    if (result.status === "fulfilled") {
      ...
      if (result.value.verdict === "warn" || result.value.verdict === "fail") needsReview = true;
      continue;
    }
