...
import type { CampaignBrief, CampaignModuleOutput, McpSource } from "../../campaign/types";
...
const moduleOutput: CampaignModuleOutput = {
  module: "x",
  title: "X Campaign",
  summary: "Posts for X",
  sections: [
...
const moduleOutputClean: CampaignModuleOutput = {
  ...moduleOutput,
  sections: [
    {
      title: "Main posts",
...
const mockFn = completeStructuredPromptWithRecovery as ReturnType<typeof vi.fn>;

const getPrompt = () => {
  const [, callInput] = mockFn.mock.calls[0] as [unknown, { prompt: string }];
